...
struct DemoState {
    lumen::VectorModel<DemoRow> source;
    ...
    DemoState() : filtered(source), paged(filtered, 5) {
        source.Key([](const DemoRow& row) { return row.id; });
        source.Map([](const DemoRow& row, lumen::ItemRow& out) {
            out.text = row.task;
    ...
    void ApplyFilters(bool reset_page) {
        ...
        filtered.Where([this, query, owner_state, status_state, enabled_state](size_t index, const lumen::ItemRow&) {
            ...
            if (!owner_state.text.empty() && Lower(row.owner).find(Lower(owner_state.text)) == std::wstring::npos)
                return false;
            if (!status_state.values.empty() &&
                std::find(status_state.values.begin(), status_state.values.end(), row.status) == status_state.values.end())
                return false;
...
int lumen_main(std::span<const std::wstring_view>) {
    return lumen::Run(L"Table filters + paging", [](lumen::Window& window) {
        using namespace lumen;
        auto state = std::make_shared<DemoState>();
        auto& root = window.Root();
        root.Padding(24.0f, 20.0f).Spacing(12.0f);
        root.Add<Label>(L"Table · external filters + paging", TextRole::Title);
        root.Add<Label>(L"Table stores filter state only; FilteredModel projects rows and PagedModel windows the result.",
                        TextRole::Caption).Secondary(true).Wrap(true);

        auto& tools = root.Add<Row>().Spacing(8.0f).AlignCross(StackPanel::CrossAlign::Center);
        state->search = &tools.Add<TextBox>().Placeholder(L"Search tasks, status, owner").Grow();
        auto& filter_button = tools.Add<Button>(L"Filters", ButtonKind::Subtle).Glyph(icon::kFilter);
        Button* filter_anchor = &filter_button;
        state->range = &tools.Add<Label>(L"", TextRole::Caption).Secondary(true);

        state->chips = &root.Add<WrapPanel>().Gap(6.0f);
        state->table = &root.Add<Table>().FillCross().Grow().RowHeight(36.0f);
        state->table->Bind(state->paged);
        state->table->AddColumn(L"Task");
        state->table->AddColumn(L"Status", 120.0f).Filterable(TableFilterKind::Choice)
        ...
        state->empty = &root.Add<EmptyState>();
        state->empty->Title(L"No matching rows").Hint(L"Clear search or remove a filter.").Visible(false);

        auto& footer = root.Add<Row>().Spacing(10.0f).AlignCross(StackPanel::CrossAlign::Center);
        state->pager = &footer.Add<Pagination>();
        footer.Add<Label>(L"Paging is optional; large local tables can stay fully virtualized.", TextRole::Caption)
            .Secondary(true).Grow();
        ...
        DemoState* raw = state.get();
        filter_button.OnClick([state, &window, filter_anchor] { window.ShowFlyout(state->filters, filter_anchor); });
        state->search->OnTextChanged([state](std::wstring_view) { state->ApplyFilters(true); });
        state->table->OnFilterChanged([raw](int, TableFilterState) { raw->ApplyFilters(true); });
        state->owner_filter->OnTextChanged([raw](std::wstring_view text) {
            TableFilterState filter; filter.text = text;
        ...
        state->empty->Action(L"Clear filters", [raw] {
            if (raw->search) raw->search->Text(L"");
            if (raw->owner_filter) raw->owner_filter->Text(L"");
            if (raw->status_filter) raw->status_filter->SelectedIndex(0);
            if (raw->enabled_filter) raw->enabled_filter->SelectedIndex(0);
            raw->table->ClearFilters();
            raw->ApplyFilters(true);
        });
